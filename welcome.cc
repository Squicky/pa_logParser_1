    ...
    while (ende == false) {
        ...
        if (10 < x) {
            if (php_recv == NULL) {
                ...
                printf("recv: id %9d %9d %9d t %9ld %9ld b %9d | ", php_recv->train_id, php_recv->train_send_countid, php_recv->paket_id, php_recv->recv_time.tv_sec, php_recv->recv_time.tv_nsec, total_recv);
            }

            if (php_send == NULL) {
                printf("send: NULL            \n");
            } else {
                printf("send: id %9d %9d %9d t %9ld %9ld b %9d \n", php_send->train_id, php_send->train_send_countid, php_send->paket_id, php_send->send_time.tv_sec, php_send->send_time.tv_nsec, total_send);
            }
        ...
        if (php_recv == NULL || php_send == NULL) {
            ...
            if (php_recv != NULL) {
                ...
                //                printf(" t: %d   | p: %d\n", ph_recv.train_id, ph_recv.paket_id);
                ...
                //                printf(" t: %d   | p: %d\n", ph_send.train_id, ph_send.paket_id);
